...
import { FIRST_WORD_DATE } from "@/utils/data/gameConfig";
...
export default function CalendarPiker() {
  ...
  const { data, isLoading: isUserGamesLoading } = useGetUserGamesByRange(
    range.start,
    range.end,
    user?.id,
  );
  ...
  if (isUserLoading || isUserGamesLoading)
    return <Skeleton className="rounded-md w-60 h-60" />;
